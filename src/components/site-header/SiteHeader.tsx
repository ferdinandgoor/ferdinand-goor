  ...
  return (
    <header className={`site-header site-header--${universe}${scrolled ? " is-scrolled" : ""}`}>
      <Container className="site-header__inner">
        ...
        <nav id="site-navigation" className={`site-header__nav${menuOpen ? " is-open" : ""}`} aria-label={`Navigation FERD ${universe === "films" ? "Films" : "Process"}`}>{universe === "films" ? filmsNavigation : processNavigation}</nav>
      </Container>
    </header>
  );
};
